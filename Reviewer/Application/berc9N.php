<?php
// Try to reuse existing DB connection; include config only if needed
if (!isset($conn)) {
    include '../../config.php';
}
...
// Determine application id (may be provided by including page)
$id = isset($id) ? (int)$id : (isset($_GET['id']) ? (int)$_GET['id'] : 0);

// Fetch data from `berc1` table for the given application id; fallback to latest if none
if ($id > 0) {
    $sql = "SELECT research_title, researcher_name, part_a_supervisor_name, department_address FROM berc1 WHERE id = ?";
    ...
    if ($stmt) {
        ...
        if ($result && $result->num_rows > 0) {
            ...
            $data = [
                'research_title' => 'N/A',
                'researcher_name' => 'N/A',
                'part_a_supervisor_name' => 'N/A',
                'department_address' => 'N/A'
            ];
        }
        $stmt->close();
    } else {
    ...
    $sql = "SELECT research_title, researcher_name, part_a_supervisor_name, department_address FROM berc1 ORDER BY id DESC LIMIT 1";
    $stmt = $conn->prepare($sql);
    if ($stmt) {
        $stmt->execute();
        $result = $stmt->get_result();
...
// Try to fetch reviewer_application id for the logged-in reviewer (if any)
$reviewer_application_id = null;
if (!empty($_SESSION['user_id']) && $id > 0) {
    $raSql = "SELECT id FROM reviewer_application WHERE application_id = ? AND reviewer_id = ? LIMIT 1";
    $raStmt = $conn->prepare($raSql);
    if ($raStmt) {
        $raStmt->bind_param('ii', $id, $_SESSION['user_id']);
        $raStmt->execute();
        $raRes = $raStmt->get_result();
        if ($raRes && $raRes->num_rows > 0) {
            $raRow = $raRes->fetch_assoc();
            $reviewer_application_id = $raRow['id'];
        }
        $raStmt->close();
    }
}
...
<div class="container">
    <!-- Add `return false;` directly in onsubmit -->
    <form id="evaluationForm" action="../submitBerc9.php" method="POST" onsubmit="return false;">
        <input type="hidden" name="application_id" value="<?= htmlspecialchars($id) ?>">
        <input type="hidden" name="reviewer_application_id" value="<?= htmlspecialchars($reviewer_application_id ?? '') ?>">
        <input type="hidden" name="reviewer_id" value="<?= htmlspecialchars($_SESSION['user_id'] ?? '') ?>">
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-8">
                        <input type="text" id="title_project" name="title_project" class="form-control" readonly
                               value="<?php echo htmlspecialchars($data['research_title']); ?>">
                    </div>
                </div>
                <div class="row mb-3">
                    ...
                    <div class="col-md-8">
                        <input type="text" id="researcher_name" name="researcher_name" class="form-control" readonly
                               value="<?php echo htmlspecialchars($data['researcher_name']); ?>">
                    </div>
                </div>
                <div class="row mb-3">
                    ...
                    <div class="col-md-8">
                        <input type="text" id="supervisor_name" name="supervisor_name" class="form-control" readonly
                               value="<?php echo htmlspecialchars($data['part_a_supervisor_name']); ?>">
                    </div>
                </div>
                <div class="row mb-3">
                    <label for="faculty_state" class="col-md-4 col-form-label"><b>Department Address</b> / <i>Alamat Jabatan:</i></label>
                    <div class="col-md-8">
                        <textarea id="faculty_state" name="faculty_state" class="form-control" readonly><?php echo htmlspecialchars($data['department_address']); ?></textarea>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="row mb-2">
                    ...
                    <div class="col-md-8">
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="research_methods[]" value="Interviews" id="interviews_checkbox">
                            <label class="form-check-label" for="interviews_checkbox">Interviews / <i>Temubual</i></label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="research_methods[]" value="Focus Groups" id="focusgroup_checkbox">
                            <label class="form-check-label" for="focusgroup_checkbox">Focus Groups / <i>Kumpulan Fokus</i></label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="research_methods[]" value="Questionnaires" id="questionnaires_checkbox">
                            <label class="form-check-label" for="questionnaires_checkbox">Questionnaires / <i>Soal Selidik</i></label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="research_methods[]" value="Controlled Trial" id="trial_checkbox">
                            <label class="form-check-label" for="trial_checkbox">Controlled Trial / <i>Percubaan Terkawal</i></label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="research_methods[]" value="Observation" id="observation_checkbox">
                            <label class="form-check-label" for="observation_checkbox">Observation / <i>Pemerhatian</i></label>
                        </div>
                        <div class="form-check">
                        ...
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="research_methods[]" value="Ingestion or Tasting" id="ingestion_tasting_checkbox">
                            <label class="form-check-label" for="ingestion_tasting_checkbox">Ingestion or Tasting / <i>Pengambilan atau Pencicipan</i></label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="research_methods[]" value="Systematic Review" id="systematic_review_checkbox">
                            <label class="form-check-label" for="systematic_review_checkbox">Systematic Review / <i>Kajian Sistematik</i></label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="research_methods[]" value="Use of Personal Records" id="personal_records_checkbox">
                            <label class="form-check-label" for="personal_records_checkbox">Use of Personal Records / <i>Penggunaan Rekod Peribadi</i></label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="research_methods[]" value="Secondary Data Analysis" id="secondary_data_checkbox">
                            <label class="form-check-label" for="secondary_data_checkbox">Secondary Data Analysis / <i>Analisis Data Sekunder</i></label>
                        </div>
                        <div class="form-check">
        ...
        <div class="card">
            <div class="card-header">Section B: Subjects</div>
            <div class="card-body">
                <div class="row mb-2">
                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><textarea id="background_comment" name="background_comment" class="form-control comment-area" placeholder="Enter comments here..."></textarea></td>
                        </tr>
                        <tr>
                            ...
                            <td><textarea id="problem_statement_comment" name="problem_statement_comment" class="form-control comment-area" placeholder="Enter comments here..."></textarea></td>
                        </tr>
                        <tr>
                            <td>Objectives</td>
                            <td>
                        ...
                        <tr>
                            ...
                            <td><textarea id="expected_benefits_comment" name="expected_benefits_comment" class="form-control comment-area" placeholder="Enter comments here..."></textarea></td>
                        </tr>
                        <tr>
                            <td>Date of Research Commencement-Ending</td>
                            <td>
                            ...
                            <td><textarea id="research_dates_comment" name="research_dates_comment" class="form-control comment-area" placeholder="Enter comments here..."></textarea></td>
                        </tr>
                        <tr>
                            <td>Expected Date of Data Collection</td>
                            <td>
                                <input type="radio" name="data_collection_date" value="yes" onclick="toggleComment('data_collection_date_comment', false)"> Yes
                                <input type="radio" name="data_collection_date" value="no" onclick="toggleComment('data_collection_date_comment', true)"> No
                            </td>
                            <td><textarea id="data_collection_date_comment" name="data_collection_date_comment" class="form-control comment-area" placeholder="Enter comments here..."></textarea></td>
                        </tr>
                        <tr>
                            <td>Location Where The Project Will Be Carried Out</td>
                            <td>
                            ...
                            <td><textarea id="project_location_comment" name="project_location_comment" class="form-control comment-area" placeholder="Enter comments here..."></textarea></td>
                        </tr>
                        <tr>
                            <td>Experimental Design and Methodology</td>
                            <td>
                                <input type="radio" name="experimental_design" value="yes" onclick="toggleComment('experimental_design_comment', false)"> Yes
                                <input type="radio" name="experimental_design" value="no" onclick="toggleComment('experimental_design_comment', true)"> No
                            </td>
                            <td><textarea id="experimental_design_comment" name="experimental_design_comment" class="form-control comment-area" placeholder="Enter comments here..."></textarea></td>
                        </tr>
                        <tr>
                            <td>Inclusion & Exclusion Criteria</td>
                            <td>
                                <input type="radio" name="criteria" value="yes" onclick="toggleComment('criteria_comment', false)"> Yes
                                <input type="radio" name="criteria" value="no" onclick="toggleComment('criteria_comment', true)"> No
                            </td>
                            <td><textarea id="criteria_comment" name="criteria_comment" class="form-control comment-area" placeholder="Enter comments here..."></textarea></td>
                        </tr>
                        <tr>
                            <td>Sample Size</td>
                            <td>
                            ...
                            <td><textarea id="sample_size_comment" name="sample_size_comment" class="form-control comment-area" placeholder="Enter comments here..."></textarea></td>
                        </tr>
                        <tr>
                            <td>Flow Chart</td>
                            <td>
                            ...
                            <td><textarea id="flow_chart_comment" name="flow_chart_comment" class="form-control comment-area" placeholder="Enter comments here..."></textarea></td>
                        </tr>
                        <tr>
                            <td>Statistical Analysis</td>
                            <td>
                                <input type="radio" name="statistical_analysis" value="yes" onclick="toggleComment('statistical_analysis_comment', false)"> Yes
                                <input type="radio" name="statistical_analysis" value="no" onclick="toggleComment('statistical_analysis_comment', true)"> No
                            </td>
                            <td><textarea id="statistical_analysis_comment" name="statistical_analysis_comment" class="form-control comment-area" placeholder="Enter comments here..."></textarea></td>
                        </tr>
                    </tbody>
                </table>
            </div>
                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        <tr>
                            <td>BERC2</td>
                            <td>
                                <input type="radio" name="berc2" value="complete"> Complete
                                <input type="radio" name="berc2" value="incomplete"> Incomplete
                            </td>
                        </tr>
                        <tr>
                            <td>BERC3</td>
                            <td>
                                <input type="radio" name="berc3" value="complete"> Complete
                                <input type="radio" name="berc3" value="incomplete"> Incomplete
                            </td>
                        </tr>
                        <tr>
                            <td>BERC5</td>
                            <td>
                                <input type="radio" name="berc5" value="complete"> Complete
                                <input type="radio" name="berc5" value="incomplete"> Incomplete
                            </td>
                        </tr>
                    </tbody>
                </table>
