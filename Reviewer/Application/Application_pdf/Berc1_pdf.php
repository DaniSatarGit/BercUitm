...
// Fetch the BERC1 form details
$sql_berc1 = "SELECT * FROM berc1 WHERE id = ? LIMIT 1";
$stmt = $conn->prepare($sql_berc1);
if (!$stmt) {
...
if ($result->num_rows === 0) {
    die("No BERC1 submission found for the provided ID.");
}
...
$pdf = new FPDF();
    ...
    $pdf->Cell(50, 10, 'Calculation:', 1, 0);
    $pdf->MultiCell(0, 10, $row['calculation'], 1, 1);
    $pdf->Cell(50, 10, 'Research Flowchart:', 1, 0);
    $pdf->MultiCell(0, 10, $row['flowchart'], 1, 1);
    $pdf->Cell(50, 10, 'Statistical Analysis:', 1, 0);
    $pdf->MultiCell(0, 10, $row['statistical_analysis'], 1, 1);
    $pdf->Ln(5);
    ...
    $filename = "BERC1_Submission_$id.pdf";
    if ($action === 'download') {
