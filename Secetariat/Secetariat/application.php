...
if (isset($_POST['id']) && isset($_POST['status'])) {
    ...
    // Update the status column in the berc1 table
    $updateStatusQuery = "UPDATE berc1 SET status = ? WHERE id = ?";
    ...
    if (!$updateStatusStmt->execute()) {
        echo "Error in updating status: " . $updateStatusStmt->error;
        exit;
    }

    // Fetch the application data from the 'berc1' table
    $fetchQuery = "SELECT id, research_title, researcher_name, part_a_supervisor_name, department_address FROM berc1 WHERE id = ?";
    $fetchStmt = $conn->prepare($fetchQuery);
    if (!$fetchStmt) {
        echo "Error in preparing fetch query: " . $conn->error;
        exit;
    }
    ...
    if ($row) {
        if ($status === 'Approved') {
            // Delete from 'coordinator_application' if exists
            $deleteCoordinatorQuery = "DELETE FROM coordinator_application WHERE id = ?";
            ...
            // Insert into 'approved_application' table
            $insertQuery = "INSERT INTO approved_application (id, research_title, researcher_name, part_a_supervisor_name, department_address, submission_date, status)
                            VALUES (?, ?, ?, ?, ?, NOW(), 'approved')
                            ON DUPLICATE KEY UPDATE research_title = VALUES(research_title), researcher_name = VALUES(researcher_name), 
                            part_a_supervisor_name = VALUES(part_a_supervisor_name), department_address = VALUES(department_address), submission_date = NOW(), status = 'approved'";
            $insertStmt = $conn->prepare($insertQuery);
            $insertStmt->bind_param("issss", $row['id'], $row['research_title'], $row['researcher_name'], $row['part_a_supervisor_name'], $row['department_address']);
            $insertStmt->execute();

            echo "Succesfull update.";
        } elseif ($status === 'Proceed to Coordinator') {
            // Delete from 'approved_application' if exists
            $deleteApprovedQuery = "DELETE FROM approved_application WHERE id = ?";
            $deleteApprovedStmt = $conn->prepare($deleteApprovedQuery);
            $deleteApprovedStmt->bind_param("i", $id);
            $deleteApprovedStmt->execute();

            // Insert into 'coordinator_application' table
            $insertQuery = "INSERT INTO coordinator_application (id, research_title, researcher_name, part_a_supervisor_name, department_address, submission_date, status, forwarded_date)
                            VALUES (?, ?, ?, ?, ?, NOW(), 'Pending', NOW())
                            ON DUPLICATE KEY UPDATE research_title = VALUES(research_title), researcher_name = VALUES(researcher_name), 
                            part_a_supervisor_name = VALUES(part_a_supervisor_name), department_address = VALUES(department_address), 
                            submission_date = NOW(), status = 'Pending', forwarded_date = NOW()";
            $insertStmt = $conn->prepare($insertQuery);
            $insertStmt->bind_param("issss", $row['id'], $row['research_title'], $row['researcher_name'], $row['part_a_supervisor_name'], $row['department_address']);
            $insertStmt->execute();
            ...
            // Insert into rejected_application table
            $insertQuery = "INSERT INTO rejected_application (id, research_title, researcher_name, part_a_supervisor_name, department_address, submission_date, status, forwarded_date)
        ...
        echo "Failed to fetch application data.";
    }
    exit;
}

// Fetch data from the 'berc1' table
$sql = "SELECT id, research_title, researcher_name, part_a_supervisor_name, department_address, status FROM berc1";
                ...
                <?php if ($result->num_rows > 0): ?>
                    ...
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td data-label="Supervisor Name"><?= htmlspecialchars($row['part_a_supervisor_name']) ?></td>
                            <td data-label="Department Address"><?= htmlspecialchars($row['department_address']) ?></td>
