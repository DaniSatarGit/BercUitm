...
// Build the base SQL query to fetch exemptions along with their assigned reviewer (if any)
$sql = "SELECT ca.id, ca.research_title, ca.researcher_name, ca.supervisor_name, ca.dept_address, ca.status, 
               ra.reviewer_id, r.name AS reviewer_name
        FROM coordinator_exemption ca
        LEFT JOIN reviewer_exemption ra ON ca.id = ra.exemption_id
...
if (!empty($filterSupervisor)) {
    $sql .= " AND ca.supervisor_name LIKE ?";
...
<head>
    ...
    <title>Exemtion Assingg Reviewer Management</title>
...
<body>
    <div class="container">
        <header>
            <h1>Exemption Reviewer Management</h1>
            ...
            <table>
                ...
                <tbody>
                    <?php if ($result->num_rows > 0): ?>
                        ...
                        <?php endwhile; ?>
                    <?php else: ?>
                        <tr><td colspan="9" class="no-results">No exemptions found.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </section>
    ...
    <script>
        $(document).ready(function() {
            $('.update-btn').on('click', function() {
                ...
                $.ajax({
                    type: 'POST',
                    url: 'assign_reviewer_exemption.php',
